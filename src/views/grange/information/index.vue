    ...
    <div v-if="isInsider!==`0`">
      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column label="创建时间" sortable prop="createdTime" align="center">
          ...
          <el-table-column label="商品名称" sortable prop="name" align="center">
            <template slot-scope="scope">
              <p>{{ scope.row.name }}</p>
            </template>
          </el-table-column>
    ...
    <div v-if="fefresh===1">
      <el-card class="operate-container" shadow="never">
        <el-select
          ...
          style="float: right"
          placeholder="排序 "
          @change="fetchData(undefined,grangeId)"
        >
          <el-option
            v-for="item in operates"
            :key="item.value"
      ...
      <div class="table-container">
        <el-table
          ...
          :default-sort="{prop: 'createdTime', order: 'desc'}"
          @sort-change="fetchData(undefined,grangeId)"
        >
          <template slot="empty">
            <img class="data-pic" src="#" alt="">
          </template>
      ...
      <div style="text-align: center">
        <el-pagination
          ...
          layout="total, prev, pager, next"
          :total="total"
          @size-change="fetchData(undefined,grangeId)"
          @current-change="fetchData(undefined,grangeId)"
        />
      </div>
    </div>
    <el-dialog
...
<script>
import { fachData, itemDelete, addGoods, permissionFarm } from '@/api/grange'
export default {
  data() {
    return {
      list: [],
  ...
  methods: {
    permission() {
      ...
      this.grangeId = window.localStorage.getItem('grange')
      ...
      if (this.isInsider === `0`) {
        permissionFarm().then(res => {
          ...
          if (grangeId !== null) {
            ...
            this.fefresh = 1
            this.operateType = 'default'
            this.fetchData('default', grangeId)
          }
          if (grangeId === null) {
    ...
    clickOnSubmit(id) {
      ...
      this.operateType = 'default'
      this.fetchData('default', window.localStorage.getItem('grange'))
    },
    permissionClose() {
      this.permissionVisble = false
    },
    ...
    handleAddProduct() {
      this.$router.push({ path: 'grangeAdd' })
    },
    handleDelete(id) {
      this.$confirm('此操作将永久删除此商品, 是否继续?', '提示', {
        ...
        itemDelete(id).then(res => {
          if (res.data.success === true) {
            if (window.localStorage.getItem('grange') !== null) {
              this.fetchData('default', window.localStorage.getItem('grange'))
            }
            if (window.localStorage.getItem('grange') === null) {
              this.fetchData()
    ...
    handleUpdateProduct(id) {
      this.$router.push({ name: '农庄编辑商品', query: { id: id }})
    },
    operateChange(val) {
      console.log(val, 'val')
    }
