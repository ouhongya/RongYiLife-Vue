...
<script>
import { categoryFetchData, addCategory } from '@/api/grange'
export default {
  data() {
    return {
      key: {
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (window.localStorage.getItem('grange') !== null) {
        categoryFetchData(window.localStorage.getItem('grange')).then(res => {
          this.loading = false
    ...
    onSubmit() {
      ...
      addCategory(commodity).then(res => {
        ...
        if (res.data.success === true) {
          ...
          this.$router.push({ path: '/grange/grangeinformation' })
        } else {
          this.$message({
            type: 'info',
    ...
    /* checkImageWH(file, resolve, reject) {
      ...
      filereader.onload = e => {
        ...
        image.onload = function() {
          if ((this.width !== 200) || this.height !== 142) {
            _this.$message.error(`请上传图片高度为200px宽度为142px的图片`)
            reject()
          } else {
            resolve()
          }
    ...
    handleBeforeUploadsbaby(file) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        // console.log(file);
        if (!(isJPG || isJPEG || isPNG)) {
          that.$message.error('只能上传JPG 、JPEG 、 PNG格式的图片~',)
          reject()
        } else if (!bigLt2M) {
          that.$message(`图片大小超过20M`)
          reject()
        } else {
