...
<script>
import { fetchData, permissionFarm } from '@/api/grangeOrder'
export default {
  data() {
    return {
      loading: false,
  ...
  created() {
    ...
    if ('WebSocket' in window) {
      ...
      if (window.localStorage.getItem('grange') !== null) {
        companyId = window.localStorage.getItem('grange')
      } else {
  ...
  methods: {
    ...
    handleDeliveryOrder(id) {
      this.$router.push({ name: '农庄订单发货', query: { id: id }})
    },
    handleViewOrder(id) {
      this.$router.push({ name: '农庄订单详情', query: { id: id }})
    },
    handleClose() {},
    initWebSocket() {
