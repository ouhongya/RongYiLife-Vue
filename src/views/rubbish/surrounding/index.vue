    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button size="small" icon="el-icon-edit" @click="compilation(scope.row.name)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--弹框-->
...
export default {
  ...
  methods: {
    ...
    // 第一个选项
    cityCreate() {
      this.$axios.get(`/server/api/city/all`).then((res) => {
    ...
    handleItemChange(val) {
      ...
      for (let i = 0; i < optionList.length; i++) {
        if (optionList[i].cities.length === 0 && val[0] != null) {
          // 二级菜单
          this.$axios.get(`/server/api/area/${val[0]}`).then(res => {
    ...
    city(value) {
      // 发送请求刷新数据 市 区 小区
      this.$axios.get(`/server/api/user/${value[0]}/${value[1]}`).then((res) => {
