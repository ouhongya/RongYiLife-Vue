...
<template>
  ...
  >
    <div class="block " style="padding-right:5px;text-align: center;">
      ...
      <el-button size="medium" icon="el-icon-plus" type="primary" @click="add">新建农庄</el-button>
    </div>
    <el-table
      :data="tableData"
      style="width: 100%"
    ...
    >
      <el-form ref="pojo" label-width="80px" :model="pojo" :rules="rules">
        <el-form-item label="农庄名称" prop="name">
          <el-input v-model="pojo.name" placeholder="请输入农庄名称" />
        </el-form-item>
        <el-form-item label="排序数值" prop="defaultSort">
          <el-input v-model="pojo.defaultSort" type="number" :min="0" placeholder="请输入排序数值" />
        </el-form-item>
        ...
        <el-form-item label="农庄地址" prop="address">
          <el-input v-model="pojo.address" placeholder="请输入农庄地址" />
        </el-form-item>
        <el-form-item label="所属账号">
          <el-input v-model="pojo.usepws" disabled />
        </el-form-item>
        <el-form-item style="text-align: center">
          <el-button size="medium" type="primary" @click="formVisible = false">关闭</el-button>
    ...
    <el-dialog
      ...
      style="text-align: center"
    >
      <el-form ref="addpojo" label-width="80px" :model="addpojo" :rules="rules">
        <el-form-item label="农庄名称" prop="name">
          <el-input v-model="addpojo.name" placeholder="请输入农庄名称" />
        </el-form-item>
        <el-form-item label="排序数值" prop="defaultSort">
          <el-input v-model="addpojo.defaultSort" type="number" :min="0" placeholder="请输入排序数值" />
        </el-form-item>
        <el-form-item label="选择地址" prop="name">
          <el-cascader
            v-model="community"
...
export default {
  ...
  methods: {
    ...
    fetchData(val) {
      ...
      this.loading = true
      let url = ''
      url = `/server/grange/all?page=${this.currentPage}&size=${this.size}&search=${this.input}&sortByFiled=${name}&sortByOrder=${order}`
    ...
    compilation(id) {
      ...
      this.$axios.get(`/server/grange/grange/${id}`).then(res => {
        this.loading = false
    ...
    addsave(addpojo) {
      this.$refs[addpojo].validate((valid) => {
        ...
        if (this.addpojo.defaultSort === '' || this.addpojo.defaultSort === undefined) {
          this.$message({
            type: 'warning',
            message: '请输入排序数值'
          })
        ...
        this.addformVisible = false
        this.loading = true
        this.$axios.post(`/server/grange/add`, this.addpojo).then(res => {
          this.addpojo = {}
          this.closeSave()
          this.fetchData()
          this.loading = false
    ...
    save(pojo) {
      this.$refs[pojo].validate((valid) => {
        ...
        if (this.pojo.defaultSort === '' || this.pojo.defaultSort === undefined) {
          this.$message({
            type: 'warning',
            message: '请输入排序数值'
          })
        ...
        this.pojo = {}
        this.loading = true
        this.$axios.post(`/server/grange/update`, user).then(res => {
          this.fetchData()
          this.closeSave()
          this.loading = false
          if (res.data.code === 10000) {
    ...
    /* 删除*/
    strikeout(id) {
      this.$confirm('此操作将永久删除此农庄, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.loading = true
        this.$axios.delete(`/server/grange/delete/${id}`).then(res => {
          this.fetchData()
          this.loading = false
          if (res.data.code === 10000) {
            this.$message({
    ...
    applicationFarm(id) {
      this.$confirm('此操作将此农场在手机端不显示, 是否继续?', '提示', {
        ...
        this.$axios.put(`/server/grange/apply/down/${id}`).then(res => {
    ...
    sortSubmit(id) {
      ...
      this.tableData.forEach(item => {
        if (item.id === id) {
          ...
          this.$axios.put(`/server/grange/sort/${obj.id}/${obj.sort}`).then(res => {
            this.loading = false
            if (res.data.success === true) {
              this.fetchData()
              item.visible2 = false
