    ...
    <div style="width: 50%;float: left;">
      <el-card class="form-container" style="width: 100%;text-align: center;line-height: 60px;height: 100%" shadow="never">
        ...
        >
          ...
          <i class="el-icon-warning" style="font-size: 10px;color: #8c939d">提示：请上传1张360*266 分辨率的png、jpg、jpeg图，单张图的容量不超过20MB，作为该店铺的产品缩略图。</i>
...
<script>
import { fetchDataId, operation, center } from '@/api/reviewShop'
export default {
  data() {
    return {
    ...
    fetchData() {
      ...
      fetchDataId(id).then(res => {
        ...
        for (let x = 0; x < data[0].imgUrls.length; x++) {
          if (data[0].imgUrls[x].location !== -1 && data[0].imgUrls[x].location !== 0) {
            this.banner.push(data[0].imgUrls[x].url)
            const urlList = {}
            urlList.url = data[0].imgUrls[x].url
            arrUrl.push(urlList)
          }
          if (data[0].imgUrls[x].location === 0) {
            const cc = {}
            cc.url = data[0].imgUrls[x].url
            this.Productthumbnail = data[0].imgUrls[x].url
    ...
    onSubmit(id) {
      ...
      operation(user).then(res => {
        ...
        if (res.data.success === true) {
          this.$router.push({ path: '/review/reviewsurrounding' })
          this.$message({
            type: 'success',
