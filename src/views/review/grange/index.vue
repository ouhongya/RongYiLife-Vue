...
<script>
import { fetchData, center, reviewfarm, operation } from '@/api/reviewGrange'
export default {
  data() {
    return {
      list: [],
  ...
  methods: {
    ...
    fetchData() {
      ...
      if ('WebSocket' in window) {
        this.websocket = new WebSocket(this.GLOBAL.BASE_URL + this.GLOBAL.grangeId)
        this.initWebSocket()
      }
    },
    handleAddProduct() {
    ...
    handleUpdateProduct(id, pass) {
      this.$router.push({ name: '农庄商品详情', query: { id: id, state: pass }})
    },
    reviewfarm(id) {
      const user = {}
      const arr = []
    ...
    History() {
      this.$router.push({ path: '/review/grangeHistory' })
    },
    initWebSocket() {
