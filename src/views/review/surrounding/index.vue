      ...
      >
        ...
        <el-table-column label="商品描述" align="center">
          <template slot-scope="scope">
            ...
            >
              <el-button slot="reference" type="text" style="color: rgb(90, 94, 102);">{{ scope.row.description.substring(0,15) }}...</el-button>
            </el-popover>
            <!--<p>品牌：{{scope.row.brandName}}</p>-->
          </template>
        </el-table-column>
...
<script>
import { fetchData, center, reviewfarm, operation } from '@/api/reviewShop'
export default {
  data() {
    return {
      list: [],
  ...
  created() {
    ...
    if ('WebSocket' in window) {
      this.websocket = new WebSocket(this.GLOBAL.BASE_URL + this.GLOBAL.storeId)
      this.initWebSocket()
    }
  },
  destroyed() {
  ...
  methods: {
    ...
    handleUpdateProduct(id, pass) {
      this.$router.push({ name: '商铺商品详情', query: { id: id, state: pass }})
    },
    reviewfarm(id) {
      const user = {}
      const arr = []
    ...
    History() {
      this.$router.push({ path: '/review/shopHistory' })
    },
    initWebSocket() {
