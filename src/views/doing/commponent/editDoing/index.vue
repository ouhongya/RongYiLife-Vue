...
<script>
import { updateDong, getDoing } from '@/api/doingRomulgation'
export default {
  name: 'IndexVue',
  data() {
    return {
  ...
  created() {
    ...
    this.loading = true
    getDoing(id).then(res => {
      this.loading = false
      if (res.data.success === true) {
        const data = res.data.queryResult.data
        this.option.forEach(val => {
          val.areas.forEach(key => {
            if (key.id === data.areaId) {
              this.community = [val.id, key.id]
            }
          })
        })
        if (data.contents.length === 1) {
          this.counter[0].count = data.contents[0].content
        } else {
  ...
  methods: {
    onSubmit() {
      console.log(this.banner.length, 'this.banner/??')
