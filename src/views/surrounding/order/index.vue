...
<script>
import { fetchData, permissionFarm } from '@/api/shopOrder'
export default {
  data() {
    return {
      loading: false,
  ...
  created() {
    ...
    if ('WebSocket' in window) {
      ...
      if (window.localStorage.getItem('store') !== null) {
        companyId = window.localStorage.getItem('store')
      } else {
  ...
  methods: {
    ...
    handleDeliveryOrder(id) {
      this.$router.push({ name: '商铺订单发货', query: { id: id }})
    },
    handleViewOrder(id) {
      this.$router.push({ name: '商铺订单详情', query: { id: id }})
    },
    handleClose() {},
    initWebSocket() {
