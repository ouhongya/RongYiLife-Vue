<template>
  ...
  >
    <div style="width: 50%;float: left;">
      <el-card class="form-container" style="width: 100%;text-align: center;line-height: 60px;height: 100%" shadow="never">
        ...
        >
          ...
          <i class="el-icon-warning" style="font-size: 10px;color: #8c939d">提示：请上传1-10张750*750 分辨率的png、jpg、jpeg图，单张图的容量不超过20MB，作为该产品的展示图。</i>
          <el-form-item label="商品缩略图" height="90px" label-width="auto" style="width: 100%">
            <el-upload
              action="/server/farmitemimg/upload"
              multiple
              list-type="picture-card"
          ...
          <i class="el-icon-warning" style="font-size: 10px;color: #8c939d">提示：请上传1张306*266 分辨率的png、jpg、jpeg图，单张图的容量不超过20MB，作为该店铺的产品缩略图。</i>
          ...
          <i class="el-icon-warning" style="font-size: 10px;color: #8c939d">提示：请上传1张 宽750，高不限制的png，jpg，jpeg图，单张图的容量不超过20MB，作为该产品的宝贝详情图</i>
    ...
    <div style="width: 35%;float: right;">
      ...
      <el-card class="form-container" style="width: 50%;height: 100%;border: 1px solid #adb2b8" shadow="never">
...
<script>
import { fetchDataItem, categoryFetchData, itemUpdate } from '@/api/shop'
export default {
  data() {
    return {
      fileList: [],
  ...
  methods: {
    fetchData() {
      ...
      fetchDataItem(id).then(res => {
        ...
        for (let x = 0; x < dataList.imgUrls.length; x++) {
          if (dataList.imgUrls[x].location !== -1 && dataList.imgUrls[x].location !== 0) {
            this.banner.push(dataList.imgUrls[x].url)
            const urlList = {}
            urlList.url = dataList.imgUrls[x].url
            arrUrl.push(urlList)
          }
          if (dataList.imgUrls[x].location === 0) {
            const cc = {}
            cc.url = dataList.imgUrls[x].url
            this.Productthumbnail = dataList.imgUrls[x].url
            this.fileListTest.push(cc)
          }
      ...
      if (window.localStorage.getItem(`store`) !== '') {
        categoryFetchData(window.localStorage.getItem('store')).then(res => {
          this.loading = false
          const dataList = res.data.queryResult.data
          const arr = []
          for (let x = 0; x < dataList.length; x++) {
    ...
    onSubmit() {
      ...
      itemUpdate(commodity).then(res => {
        ...
        if (res.data.success === true) {
          this.$router.push({ path: '/surrounding/information' })
          this.key = {}
          this.dialogImageUrl = []
          this.Spu = {}
          this.url = ''
    ...
    test(res, file) {
      this.Productthumbnail = file.response.queryResult.data
    },
    onBeforeUpload(file) {
      const that = this
      return new Promise(function(resolve, reject) {
        ...
        const bigLt2M = file.size / 1024 < 20480
    ...
    cc(file, resolve, reject) {
      ...
      filereader.onload = e => {
        ...
        const image = new Image()
        image.onload = function() {
          if (this.width !== _this.GLOBAL.PRODUCT_STORE_Thumbnail_WIDTH || this.height !== _this.GLOBAL.PRODUCT_STORE_Thumbnail_HEIGHT) {
            _this.$message.warning(`请上传图片高度为306px宽度为266px的图片`)
            reject()
          } else {
            resolve()
          }
