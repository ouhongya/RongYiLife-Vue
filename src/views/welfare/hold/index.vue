<template>
  ...
  >
    <el-card class="box-card" style="padding: 5px">
      <FONT><strong style="float: left;color: #5a5e66">最新排行:</strong></FONT>
      <el-date-picker
        v-model="value"
        type="daterange"
        size="medium"
      ...
      >
        ...
        >
          <template slot-scope="props">
            {{ props.row.endTime|dateformat(`YYYY-MM-DD HH:mm`) }}
          </template>
        </el-table-column>
        <el-table-column
          prop="name"
...
<script>
import { fetchDataPage, fetchDataAll } from '@/api/welfareRanking'
export default {
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    lookRanking(id) {
      this.$router.push({ name: 'ranking', query: { id: id }})
    },
    addRanking() {
