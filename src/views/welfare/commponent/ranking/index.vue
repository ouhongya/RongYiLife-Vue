...
<script>
import { onsubmit, fetchDataId } from '@/api/welfareRanking'
export default {
  data() {
    return {
      form: {
  ...
  methods: {
    fetchData() {
      ...
      if (this.id !== undefined || this.id !== '') {
        fetchDataId(this.id).then(res => {
          ...
          const arrUrl = []
          const urlList = {}
          urlList.url = dataList.banner
          arrUrl.push(urlList)
          this.Lookform.fileList = arrUrl
          this.Lookform.url = dataList.banner
          this.Lookform.textarea = (dataList.introduction)
          dataList.detail.forEach(item => {
            if (item.no === 1) {
              this.Lookform.oneName = item.name
              this.Lookform.oneUrl = (item.avatar)
            }
            if (item.no === 2) {
              this.Lookform.twoName = item.name
              this.Lookform.twoUrl = (item.avatar)
            }
            if (item.no === 3) {
              this.Lookform.threeName = item.name
              this.Lookform.threeUrl = (item.avatar)
            }
          })
        })
      }
    ...
    OnSubmit() {
      ...
      object.introduction = this.form.textarea
      object.arr = arr
