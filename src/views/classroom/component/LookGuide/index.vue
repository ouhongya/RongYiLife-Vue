<template>
  <div>
    <div style="padding-right:5px;text-align: center;">
      <el-button type="primary" size="small" style="height: 38px;float: left"@click="returnTest()">返回</el-button>
      <el-button type="primary" icon="el-icon-plus" size="small" style="height: 38px;margin: 0px 0px 0px 16%"@click="addformVisible=true;category.categoryName=name">添加名称</el-button>
      <el-input v-model="input" placeholder="请输入搜索名称" style="width: 300px;float: right" />
    </div>
    <el-table
      :data="createList"
      style="width: 100%"
    >
      ...
      >
        <template slot-scope="props">
          <el-button type="primary" size="small" @click="categoryStrike(props.row.id)">编辑</el-button>
          <el-button type="danger" size="small" @click="strikeCategory(props.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    >
      ...
      <el-button style="margin-top: 12px;" class="buttontest" type="primary" size="small" @click="close">取消</el-button>
      <el-button style="margin-top: 12px;" class="buttontest" type="primary" size="small" @click="categorySend()">确定</el-button>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    returnTest() {
      this.$router.push({ path: '/classroom/guide' })
    }
  }
}
</script>
