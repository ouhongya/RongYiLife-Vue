<template>
  ...
  >
    ...
    <el-card v-if="cityId!==''" class="tableCard">
      <el-button type="primary" style="margin: 0px 0px 0px 45%;font-size: 15px;" size="small" @click="formVisible=true;formData.color='#06ff40';color='#06ff40'">添加类别</el-button>
      <el-table
        :data="list"
        style="width: 100%"
        @expand-change="test"
      >
        ...
        >
          <template slot-scope="scope">
            <span />
            <el-image
              :style="{'background-color':scope.row.color,width:`100px`,height:`120px`}"
              :src="scope.row.url"
        ...
        >
          <template slot-scope="props">
            ...
            <el-button v-if="props.row.num===`0`" type="danger" size="small" @click="strike(props.row.id)">删除</el-button>
            <el-button v-if="props.row.num!==`0`" type="danger" size="small" disabled @click="strike(props.row.id)">删除</el-button>
            <el-button type="primary" size="small" @click="toRouter(props.row.id,props.row.name)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    >
      <el-form ref="formData" :model="formData" label-width="80px">
        ...
        <el-form-item label="图片:">
          <el-upload
            ...
            action="/server/garbageCategory/upload"
        ...
        <i class="el-icon-warning" style="font-size: 10px;color: #8c939d">提示：请上传1张200*274 分辨率的png、jpg、jpeg图，单张图的容量不超过20MB，作为该垃圾的展示图</i>
      ...
      <el-button style="margin-top: 12px;" class="buttontest" size="small" @click="close">取消</el-button>
      <el-button style="margin-top: 12px;" class="buttontest" size="small" @click="send(`formData`)">确定</el-button>
    </el-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      if (this.formData.url === '' || this.formData.url === undefined) {
        this.$message({
          type: 'warning',
          message: '请上传一张垃圾分类示意图'
        })
        return
      }
    ...
    handleAvatarSuccess(res, file) {
      this.formData.url = res.queryResult.data
    },
...
<style>
  ...
  #uplocdurl i.avatar-uploader-icon {
    font-size: 28px;
    color: #8c939d;
    width: 100px;
    height: 150px;
